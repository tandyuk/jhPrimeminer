...
int esprintf_d(char *out, int value, int padRight, int padZero, int width){
	...
	if( width >= 0 ){
		...
		if( width < totalDigits ) totalDigits = width;
		if( dl >= totalDigits )	negative = 0;
		int NumberDigits = totalDigits - negative;
		int PadDigits = width - totalDigits;
		char PadDigit = ' ';
		if( padZero ) PadDigit = '0';
		//Negative sign first
		if( negative ){
			out[r] = '-'; r++;
		}
...
int esprintf_B(char *out, bool value, int padRight, int padZero, int width){
	if(value){
		*out = '1';
	}else{
		*out = '0';
...
int esprintf_X(char *out, unsigned int value, int padRight, int padZero, int width, int UpperCase){
	...
	int dl = 0; //HexLength
	while(value){
		if( UpperCase )	DezimalStr[dl] = esprintf_HEX_UPPER[value&0xF];
		else DezimalStr[dl] = esprintf_HEX_LOWER[value&0xF];
		dl++;			
		value >>= 4;
	}
	if( width >= 0 ){
...
void esprintf(char *out, char *format, ...){
  ...
  unsigned int *lengthOut = 0;
	...
	*o = '\0';
	if( lengthOut )	*lengthOut = (unsigned int)(o-out);
}
